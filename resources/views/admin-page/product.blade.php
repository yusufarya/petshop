    ...
    <div class="container-fluid">
        <div class="row mx-2">
          <div class="row justify-content-end mb-2 w-100">
            <a href="/products/create" class="btn float-right btn-add "><i class="fas fa-plus-square"></i> &nbsp; Data</a>
          </div>
          <table class="table table-bordered table-sm">
              <thead>
                  <tr class="my-bg-primary text-white">
                      ...
                      <th style="width: 11%">Standar Beli</th>
                      <th style="width: 11%">Standar Jual</th>
              ...
              <tbody>
                @foreach ($resultData as $row)
                  <tr>
                      ...
                      <td>{{ $row->units->initial }}</td>
                      <td>{{ $row->sizes->initial }}</td>
                      <td>{{ rupiah($row->purchase_price) }}</td>
                      <td>{{ rupiah($row->selling_price) }}</td>
                      <td style=" text-align: center;">
                        <a href="/products/{{$row->id}}/edit" class="text-warning"><i class="fas fa-edit"></i></a>
                        &nbsp;
